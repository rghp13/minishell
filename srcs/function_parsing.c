...
int	function_parse(t_command *command, char *line)
{
	int	i;
	int	j;
	...
	while (line[i])
	{
		if (is_redirector(line[i], line[i + 1]) || line[i + 1] == '\0')
		{
			if (small_malloc(&command->command, sizeof(char) * ((i - j) + 1)))
				return (-1);
			memcpy(command->command, &line[j], i - j);
			command->command[i - j] = '\0';
			command->redirector = is_redirector(line[i], line[i + 1]);
			i += 1 + (is_redirector(line[i], line [i + 1]) > 2);
			j = i;
			while (line[i])
			...
			if (i != j)
			{
				if (small_malloc(command->file, sizeof(char) * ((i - j) + 1)))
					return (-1);
				memcpy(command->file, &line[j], i - j);
				command->file[i - j] = '\0';
			}
			break ;
		}
